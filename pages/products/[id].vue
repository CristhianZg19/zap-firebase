<template>
    <div class="min-h-screen bg-gray-100 p-8">
      <div class="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow-lg">
        ...
        <div v-if="loading" class="text-center text-gray-500 mt-6">Cargando...</div>
        <div v-if="error" class="text-center text-red-500 mb-4 mt-6">{{ error }}</div>
  ...
  const prevSlide = () => {
    if (imageList.value.length > 0) {
      currentSlide.value = (currentSlide.value - 1 + imageList.value.length) % imageList.value.length;
    }
  };
  </script>
